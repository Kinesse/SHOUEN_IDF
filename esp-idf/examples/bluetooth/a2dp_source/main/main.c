...
void app_main()
{
    ...
    host.max_freq_khz = SDMMC_FREQ_PROBING/2.5;         //new addition
...
static void filter_inquiry_scan_result(esp_bt_gap_cb_param_t *param)
{
    ...
    /* search for device named "TicPods 3144" in its extended inqury response */
    if (eir) {
        get_name_from_eir(eir, peer_bdname, NULL);
        if (strcmp((char *)peer_bdname, "TicPods 3144") != 0) {
            return;
        }
